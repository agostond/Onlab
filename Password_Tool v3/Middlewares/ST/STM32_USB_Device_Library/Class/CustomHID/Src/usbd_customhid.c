...
//defineok
uint8_t sendfeature[USB_REPORT_SIZE];

void USB_Clear_Feature(){
	memset(sendfeature, 0, sizeof(sendfeature));
}


uint8_t USB_Set_Feature(uint8_t newfeature[], uint8_t size){
	uint8_t i;
	if(size > USB_REPORT_SIZE){
		return 0;
	}
	if(newfeature != NULL){
		if(sizeof(newfeature) <= sizeof(sendfeature)){
			for(i = 0; i < USB_REPORT_SIZE; i++){
				sendfeature[i] = newfeature[i];
			}
			return 1;
		}
	}
    ...
    case USB_REQ_TYPE_CLASS :
      switch (req->bRequest)
      {
        ...
        case CUSTOM_HID_REQ_GET_REPORT:
          USBD_CtlSendData(pdev, (uint8_t *)(void *)sendfeature, sizeof(sendfeature));
          break;
